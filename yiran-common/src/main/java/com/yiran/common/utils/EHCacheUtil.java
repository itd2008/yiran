...
public class EHCacheUtil {
	...
	public static boolean setValue(String cacheName, String key, Object value,Integer timeToIdleSeconds, Integer timeToLiveSeconds) throws Exception {
		try {
			CacheManager myManager = CacheManager.create();
			Cache myCache = myManager.getCache(cacheName);
			if (myCache == null) {
			...
			myCache.put(new Element(key, value, EHCacheConfig.ETERNAL, timeToIdleSeconds, timeToLiveSeconds));
			return true;
		} catch (Exception e) {
			throw new Exception("set cache " + cacheName + " failed!!!");
		}
	...
	public static boolean setValue(String cacheName, String key, Object value, Integer timeToLiveSeconds) throws Exception {
		return setValue(cacheName,key,value, EHCacheConfig.TIMETOIDLESECONDS,timeToLiveSeconds);
	}
