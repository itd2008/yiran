...
(function ($) {
    $.extend({
    	...
    	table: {
			...
			imageView: function (value, path, target) {
				...
				if ($.common.isNotEmpty(value)) {
					return $.common.sprintf("<img class='img-circle img-xs' data-target='%s' src='%s'/>", _target, value);
				} else {
        ...
        operate: {
            ...
            // 上传文件
            upload: function(id) {
            	var url = $.common.isEmpty(id) ? $.table._option.uploadUrl : $.table._option.uploadUrl.replace("{id}", id);
                $.modal.openNoBtn("上传文件", url);
            },
            // 添加信息，以tab页展现
            addTab: function (id) {
            ...
            addMenuItem:function(id){
            	var url = $.common.isEmpty(id) ? $.table._option.createUrl : $.table._option.createUrl.replace("{id}", id);
                createMenuItem(url, "添加"+$.table._option.modalName);
            },
            // 添加信息 无按钮全屏
            addNoBtnFull: function(id) {
            	var url = $.common.isEmpty(id) ? $.table._option.createUrl : $.table._option.createUrl.replace("{id}", id);
                $.modal.openNoBtnFull("添加" + $.table._option.modalName, url);
            },
            // 添加访问地址
            addUrl: function(id) {
            	var url = $.common.isEmpty(id) ? $.table._option.createUrl : $.table._option.createUrl.replace("{id}", id);
            ...
            sendGood: function(id) {
            	if($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = $.table._option.setSendGoodUrl.replace("{id}", row[$.table._option.uniqueId]);
                    $.modal.open("订单发货", url);
            	} else {
            	    $.modal.open("订单发货" , $.operate.sendGoodUrl(id));
            	}
            },
            sendGoodUrl: function(id) {
            	var url = "/404.html";
            	if ($.common.isNotEmpty(id)) {
            	    url = $.table._option.setSendGoodUrl.replace("{id}", id);
            	} else {
            ...
            setTargetInst: function(id) {
            	if($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = $.table._option.setTargetInstUrl.replace("{id}", row[$.table._option.uniqueId]);
            ...
            setTargetUrl: function(id) {
            	var url = "/404.html";
            	if ($.common.isNotEmpty(id)) {
            	    url = $.table._option.setTargetInstUrl.replace("{id}", id);
            	} else {
            	    var id = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
            	    if (id.length == 0) {
            			$.modal.alertWarning("请至少选择一条记录");
            ...
            setFundChannelExt: function(id) {
            	if($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = $.table._option.setTargetInstUrl.replace("{id}", row[$.table._option.uniqueId]);
                    $.modal.openNoBtnFull("资金渠道基本配置 - 维护资金源 - 设置渠道特性", url);
            	} else {
            	    $.modal.openNoBtnFull("资金渠道基本配置 - 维护资金源 - 设置渠道特性" , $.operate.setFundChannelExtUrl(id));
            	}
            },
            setFundChannelExtUrl: function(id) {
            	var url = "/404.html";
            	if ($.common.isNotEmpty(id)) {
            	    url = $.table._option.setFundChannelExtUrl.replace("{id}", id);
            	} else {
            	    var id = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
            	    if (id.length == 0) {
            			$.modal.alertWarning("请至少选择一条记录");
          ...
          //设置渠道维护期
            setChannelMaintenancePeriod: function(id) {
            	if($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = $.table._option.setTargetInstUrl.replace("{id}", row[$.table._option.uniqueId]);
                    $.modal.openNoBtnFull("资金渠道基本配置 - 维护资金源 - 设置渠道维护期", url);
            	} else {
            	    $.modal.openNoBtnFull("资金渠道基本配置 - 维护资金源 - 设置渠道维护期" , $.operate.getChannelMaintenancePeriodUrl(id));
            	}
            },
            getChannelMaintenancePeriodUrl: function(id) {
            	...
            	if ($.common.isNotEmpty(id)) {
            	    ...
            	    url = $.table._option.setChannelMaintenancePeriodUrl.replace("{id}", id);
            	}
                return url;
            },
          ...
          //设置渠道维接口
            setChannelApi: function(id) {
            	if($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = $.table._option.setTargetInstUrl.replace("{id}", row[$.table._option.uniqueId]);
                    $.modal.openNoBtnFull("资金渠道基本配置 - 维护资金源 - 设置渠道接口", url);
            	} else {
            	    $.modal.openNoBtnFull("资金渠道基本配置 - 维护资金源 - 设置渠道接口" , $.operate.getChannelApiUrl(id));
            	}
            },
            getChannelApiUrl: function(id) {
            	...
            	if ($.common.isNotEmpty(id)) {
            	    ...
            	    url = $.table._option.setChannelApiUrl.replace("{id}", id);
            	}
                return url;
            },
            // 修改信息，以tab页展现
